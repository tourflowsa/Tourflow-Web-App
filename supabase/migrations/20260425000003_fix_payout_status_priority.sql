-- Migration: fix_payout_status_priority
-- Objective: Ensure 'paid' status takes priority in the financial breakdown, and provide resolution_status separately.
  ...
  LOOP
    ...
    BEGIN
      ...
      IF FOUND THEN
        ...
        -- Resolution Mapping
        IF v_ledger.adjustment_reason IS NOT NULL THEN
          DECLARE
        ...
        -- Status Logic (Priority: PAID > DISPUTED > RESOLVED)
        IF v_payout_status = 'paid' THEN
          -- Keep status as paid
        ELSIF v_is_disputed THEN
          v_payout_status := 'disputed';
          v_dispute_amount := v_dispute_amount + v_payout;
        ELSIF v_resolution_status IS NOT NULL THEN
