-- Migration: fix_dispute_visibility
-- Objective: Ensure dispute_amount and is_disputed flags are correctly mapped in the financial breakdown RPC.
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      IF FOUND THEN
        v_payout := COALESCE(v_ledger.adjusted_amount, v_ledger.amount_net, v_ledger.original_amount, 0);
        v_payout_status := v_ledger.status;
        v_is_disputed := COALESCE(v_ledger.is_on_hold, false) AND COALESCE(v_ledger.hold_reason = 'dispute', false);
        
        -- Override status if disputed
        IF v_is_disputed THEN
            v_payout_status := 'disputed';
            v_dispute_amount := v_dispute_amount + v_payout;
        ELSIF v_ledger.adjustment_reason IS NOT NULL THEN
            DECLARE
              v_original numeric := COALESCE(v_ledger.original_amount, v_ledger.amount_net, 0);
              v_adjusted numeric := COALESCE(v_ledger.adjusted_amount, 0);
            BEGIN
              IF v_adjusted < v_original THEN
                  v_resolution_status := 'resolved_reduced';
              ELSE
                  v_resolution_status := 'resolved_approved';
              END IF;

              -- Priority: PAID > RESOLVED
              IF v_payout_status != 'paid' THEN
                v_payout_status := v_resolution_status;
              END IF;
            END;
        END IF;
      ...
      v_providers := v_providers || jsonb_build_object(
        ...
        'is_disputed', v_is_disputed,
        'adjustment_reason', CASE WHEN FOUND THEN v_ledger.adjustment_reason ELSE NULL END
      );
      ...
      ELSIF v_payout_status != 'disputed' THEN
        -- Only add to outstanding if NOT disputed (disputed has its own bucket)
        v_outstanding_payout := v_outstanding_payout + v_payout;
      END IF;
    END;
  END LOOP;
    ...
    IF FOUND THEN
      ...
      BEGIN
        ...
        IF v_payout_status = 'paid' THEN
          v_total_paid_out := v_total_paid_out + v_payout;
        ELSIF v_payout_status != 'disputed' THEN
          -- Only add to outstanding if NOT disputed
          v_outstanding_payout := v_outstanding_payout + v_payout;
        END IF;
      END;
    END IF;
