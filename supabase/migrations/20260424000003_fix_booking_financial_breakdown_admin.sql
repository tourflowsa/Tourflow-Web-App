-- Migration: fix_booking_financial_breakdown_admin
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE booking_id = p_booking_id
      ...
      AND status NOT IN ('removed', 'cancelled', 'rejected')
  ) INTO v_is_assigned;
  ...
  FOR provider_record IN 
    ...
    WHERE ba.booking_id = p_booking_id
      ...
      AND ba.status NOT IN ('removed', 'cancelled', 'rejected')
  LOOP
    DECLARE
    ...
    BEGIN
      ...
      IF FOUND THEN
        ...
        v_is_disputed := COALESCE(v_ledger.is_disputed, false);
      ELSE
      ...
      IF v_is_disputed THEN
        IF FOUND AND v_ledger.amount_net IS NOT NULL THEN
          v_dispute_amount := v_dispute_amount + (v_ledger.amount_net - v_payout);
        END IF;
      END IF;
    END;
  END LOOP;
