...
import { 
  listGuideAvailabilityRequestsForGuide,
  updateGuideAvailabilityRequestStatus
} from '../../lib/bookingService';
import { GuideAvailabilityRequest } from '../../types';
import { CalendarDays, Check, X, Loader2, Clock } from 'lucide-react';

export const GuideRequestsPage: React.FC = () => {
  const { user } = useAuth();
  const [requests, setRequests] = useState<GuideAvailabilityRequest[]>([]);
  ...
  const loadRequests = async () => {
    ...
    setError(null);
    try {
      const data = await listGuideAvailabilityRequestsForGuide(user.id);
      setRequests(data);
    } catch (err: any) {
  ...
  const handleAction = async (requestId: string, status: 'accepted' | 'declined') => {
    ...
    try {
      await updateGuideAvailabilityRequestStatus(requestId, user.id, status);
