...
import { getDriverAssignments, acceptAssignment, rejectAssignment, archiveAssignmentForResource, unarchiveAssignmentForResource, getArchivedAssignmentIdsForResource } from '../../lib/assignmentService';
...
export const AssignmentsList: React.FC = () => {
  ...
  const loadAssignments = useCallback(async () => {
    ...
    try {
      const [data, archived, payoutList] = await Promise.all([
        getDriverAssignments(profile.id),
        getArchivedAssignmentIdsForResource(profile.id, 'driver'),
  ...
  const handleArchive = async (id: string) => {
    ...
    setErrorNotice(null);
    try {
      await archiveAssignmentForResource(id, profile.id, 'driver');
      await loadAssignments();
    } catch (err: any) {
      console.error(err);
          ...
          {section === 'upcoming' && isPending && (
            <div className="flex gap-2 flex-1 md:flex-none items-center">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/driver/assignments/${a.id}`);
                }}
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-brand-charcoal">Driver Assignments</h1>
          <p className="text-gray-500">View and manage your driving jobs.</p>
        </div>
