...
import { getOperatorBankDetails, saveOperatorBankDetails, OperatorBankDetails, getOperatorBankStatus } from '../lib/operatorBankDetailsService';

interface OperatorBankDetailsFormProps {
  operatorId: string;
  onSave?: () => void;
}

export const OperatorBankDetailsForm: React.FC<OperatorBankDetailsFormProps> = ({ operatorId, onSave }) => {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [showAccountNumber, setShowAccountNumber] = useState(false);
  const [details, setDetails] = useState<OperatorBankDetails | null>(null);
  ...
  const loadBankDetails = async () => {
    setLoading(true);
    try {
      const data = await getOperatorBankDetails(operatorId);
      setDetails(data);
      if (data) {
        setFormData({
      ...
      console.error('Failed to load operator bank details', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const status = getOperatorBankStatus(details);
  const statusColors = {
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              ...
              Changing bank details will trigger an audit log entry.
            </p>
          </div>
        </div>
      </div>
